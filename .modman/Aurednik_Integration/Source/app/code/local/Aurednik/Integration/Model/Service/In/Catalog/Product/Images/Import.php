...
/**
 * Klasse, die Mehtoden bereitstellt, um ein Produktbilderimport durchzuführen
 *
 * @author akniss
 *
 ...
 *
 */
class Aurednik_Integration_Model_Service_In_Catalog_Product_Images_Import extends Aurednik_Integration_Model_Service_In_Catalog_Product_Import
{
	...
	public function execute($logPerformance = true)
	{
		...
		{
			$end = microtime(true);
			$this->getHelper()->logInfoMessage('Dauer des Mapping von ' . count($products) . ' Produktbilder in Sekunden: ' . ($end - $start));
		}
	...
	protected function processData(array $products, $logImportDuration = true)
	{
		...
		if ($logImportDuration)
		{
			...
			$this->getHelper()->logInfoMessage('Dauer des Importvorgangs von ' . count($products) . ' Produktbilder in Sekunden: ' . ($endImport - $startImport));
		}
