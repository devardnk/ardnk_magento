...
class DerModPro_BasePrice_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	public function getBasePriceLabel($product, $labelFormat = false)
	{
		...
		$prodAmountAttr = Mage::getStoreConfig ('catalog/baseprice/productamountattribute', $storeID );
		$prodUnitAttr 	= Mage::getStoreConfig ('catalog/baseprice/productunitattribute', $storeID );


		// Produkt Menge für aktuellen Preis
		$productAmount 	= $product->getData($prodAmountAttr);
		if (empty($productAmount))
		{
			return '';
		}

		// Produktmengen - Einheit
		$productUnit 	= $product->getAttributeText($prodUnitAttr);

		if($productAmount < 1)
		{
			$prodUnitAmount = $this->convertProductUnitAndAmount($productUnit, $productAmount);
			$productUnit 	= $prodUnitAmount['unit'];
			$productAmount 	= $prodUnitAmount['amount'];
		}

		$refUnitAmount 	= $this->getReferencedUnitAmount($productUnit, $productAmount);
		$referenceUnit 	= $refUnitAmount['unit'];
		$referenceAmount= $refUnitAmount['amount'];

		/** ============ Modifikationen SDZeCOM - ENDE ============== */
		...
		$label = str_replace('{{product_unit_short}}', $this->__($productUnit), $label);
		$label = str_replace('{{reference_amount}}', $referenceAmount, $label);
		$label = str_replace('{{reference_unit}}', $this->__($referenceUnit), $label);
		$label = str_replace('{{reference_unit_short}}', $this->__($referenceUnit), $label);
	...
	/**
	 * Konvertierung der Messeinheit und Menge eines Produktes
	 *
	 * Bsp. 0,9 kg wird nach 900g konvertiert
	 *
	 * @author egutsche
	 *
	 * @param $productUnit
	 ...
	 * @return array
	 */
	protected function convertProductUnitAndAmount($productUnit, $productAmount)
	{
		$productAmount = trim($productAmount);
		$productAmount = str_replace(array('.', ','), array('', '.'), $productAmount);
	...
	/**
	 * Referenzmenge und Einheit für das aktuelle Produkt zurückgeben
	 *
	 * @author egutsche
	 *
	 * @param $productUnit
	 ...
	 */
	protected function getReferencedUnitAmount($productUnit, $productAmount)
	{
		...
		$referenceUnitAmount = array(
			'unit' =>$productUnit,
			'amount' => $referencedAmount
		);

		return $referenceUnitAmount;
	}
